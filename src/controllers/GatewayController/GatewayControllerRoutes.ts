...
gatewayControllerRoutes.patch("/getRecycleCoins", (request, response) => GatewayController.gainRecycleCoins(request.headers.token, request.body).then(result => response.send(result)))
